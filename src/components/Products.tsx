        ...
        <div className="grid md-grid-cols-2 lg-grid-cols-3 gap-8">
          {products.map((product, index) => (
            ...
            >
              ...
              {/* Icon */}
              <div className={`product-icon gradient-${product.gradient.includes('green') ? 'green' : product.gradient.includes('blue') ? 'blue' : product.gradient.includes('purple') ? 'purple' : product.gradient.includes('pink') ? 'pink' : product.gradient.includes('slate') ? 'blue' : 'orange'}`}>
              ...
              {product.status !== 'Coming Soon' && (
                <>
                  ...
                  {(product.androidLink || product.iosLink) ? (
                    ...
                    <button
                      onClick={() => window.open(product.link, '_blank')}
                      className={`product-btn gradient-${product.gradient.includes('green') ? 'green' : product.gradient.includes('blue') ? 'blue' : product.gradient.includes('purple') ? 'purple' : product.gradient.includes('pink') ? 'pink' : product.gradient.includes('slate') ? 'blue' : 'orange'}`}
                      style={{background: `linear-gradient(135deg, ${product.gradient.includes('green') ? 'var(--color-green-500), #14b8a6' : product.gradient.includes('blue') ? 'var(--color-blue-500), var(--color-indigo-600)' : product.gradient.includes('purple') ? 'var(--color-purple-600), #ec4899' : product.gradient.includes('pink') ? '#ec4899, #f43f5e' : product.gradient.includes('slate') ? 'var(--color-slate-700), var(--color-indigo-900)' : '#f97316, #dc2626'})`}}
                    >
